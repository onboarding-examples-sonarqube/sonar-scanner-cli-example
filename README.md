...
- The version being used for `sonarqube-scan-action` is `v5`, major version is being used in order to not have to change the version constantly and setting the version this way it will always use the latest version from the `v5` version. Ex, if `v5` set and latest is `v5.1.0`, it will use this latest and if it changes to `v5.2.0`, it will automatically use that one when it gets added.
